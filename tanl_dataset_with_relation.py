...
room_idx = {0: 'living room', 1: 'master room', 2: 'kitchen', 3: 'bathroom', 4: 'dining room', 5: 'common room 2',
            6: 'common room 3', 7: 'common room 1', 8: 'common room 4', 9: 'balcony', 10: 'entrance', 11: 'storage'}

room_type_list = ['living room', 'master room', 'kitchen', 'bathroom', 'dining room', 'common room', 'balcony',
                  'entrance', 'storage']
    ...
    for path in tqdm(train_data_path):
        ...
        if 0 < sample_index < 30:
            ...
            for idx in range(len(room_node)):
                ...
                room_node[idx]['near_x_min'] = room_node[dist_x_min.argmin()][
                    'x_max'] if dist_x_min.min() < side_width else -1
                relation += (dist_x_min < side_width).nonzero()[0].tolist()
                dist_y_min = abs(room['y_min'] - all_y_max)
                room_node[idx]['near_y_min'] = room_node[dist_y_min.argmin()][
                    'y_max'] if dist_y_min.min() < side_width else -1
                relation += (dist_y_min < side_width).nonzero()[0].tolist()
                dist_x_max = abs(room['x_max'] - all_x_min)
                room_node[idx]['near_x_max'] = room_node[dist_x_max.argmin()][
                    'x_min'] if dist_x_max.min() < side_width else -1
                relation += (dist_x_max < side_width).nonzero()[0].tolist()
                dist_y_max = abs(room['y_max'] - all_y_min)
                room_node[idx]['near_y_max'] = room_node[dist_y_max.argmin()][
                    'y_min'] if dist_y_max.min() < side_width else -1
            ...
            for room in room_node:
                ...
                if room['category'] in bedroom_idx:
                    ...
                    for rm in room_node:
                        if rm['centroid'][0] in range(min_x, max_x) and \
                                rm['centroid'][1] in range(min_y, max_y) and rm['category'] == 3:  # if rm is a bathroom
                ...
                if room['category'] in [3, 9]:
                    ...
                    room_door_mask_x = (interiordoor_pixel[0] < edge_pts[1][0] + 4) * (
                            interiordoor_pixel[0] > edge_pts[0][0] - 4)
                    room_door_mask_y = (interiordoor_pixel[1] < edge_pts[1][1] + 4) * (
                            interiordoor_pixel[1] > edge_pts[0][1] - 4)
                    if (room_door_mask_x * room_door_mask_y).sum() > 0:  # If there is a interior door
                        ...
                        room_door_pts = [(room_door_pixel[0].min(), room_door_pixel[1].min()),
                                         (room_door_pixel[0].max(), room_door_pixel[1].max())]
                        height, width = room_door_pts[1][0] - room_door_pts[0][0] + 1, room_door_pts[1][1] - \
                                        room_door_pts[0][1] + 1
                        # Check if the interior door connects to the living room
                        # Within a 3x3 window
                        connect_region_tl = category_mask[room_door_pts[0][0] - 1:room_door_pts[0][0] + 2,
                                            room_door_pts[0][1] - 1:room_door_pts[0][1] + 2]
                        connect_region_br = category_mask[room_door_pts[1][0] - 1:room_door_pts[1][0] + 2,
                                            room_door_pts[1][1] - 1:room_door_pts[1][1] + 2]
