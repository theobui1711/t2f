...
def generate_mask(new_image, last_dict):
    ...
    for key in dict_pred_mask.keys():
        for i in range(256):
            for j in range(256):
                if new_image[i, j, 0] == last_dict[key][0] \
                        and new_image[i, j, 1] == last_dict[key][1] \
                        and new_image[i, j, 2] == last_dict[key][2]:
                    dict_pred_mask[key][i, j] = 1
    return dict_pred_mask


def generate_one_dim_mask(dict_pred_mask):
    rooms = {'170,232,238': 1, '128,128,240': 2, '230,216,173': 3, '0,215,255': 4, '0,165,255': 5, '35,142,107': 6,
             '221,160,221': 7, '0,255,255': 8, '214,112,218': 9}
...
def find_central_point(one_dim_gt_mask, one_dim_pred_mask):
    ...
    for ox in o_ranges:
        for oy in o_ranges:
            IoUs[(ox, oy)] = np.average(
                jaccard_score(np.roll(np.roll(one_dim_gt_mask, ox, -1), oy, -2).flatten(), one_dim_pred_mask.flatten(),
                              average=None)[1:])
...
def calculate_iou(rooms, dict_gt_mask, dict_pred_mask, max_key):
    ...
    existed_rooms = []
    for room in rooms:
        if jaccard_score(dict_gt_mask[room].flatten(),
                         dict_gt_mask[room].flatten()) == 1.:  # if the room type does not exist in the image
    ...
    macro_iou = np.sum([intersection_union[room][0] for room in existed_rooms]) / np.sum(
        [intersection_union[room][1] for room in existed_rooms])
...
if __name__ == '__main__':
    ...
    last_5k_type = ['255,255,255', '0,0,0', '0,0,255', '170,232,238', '128,128,240', '230,216,173', '0,215,255',
                    '0,165,255', '35,142,107', '221,160,221', '0,255,255', '214,112,218']
    ...
    pred_image = cv2.imread(os.path.join(
        "/outputs/unet1_cond3.0_sample512_start256_lr1e-5_dim256_nofp16/eval_samples",
        "eval_6282.png"))
    ...
    rooms = ['170,232,238', '128,128,240', '230,216,173', '0,215,255', '0,165,255', '35,142,107', '221,160,221',
             '0,255,255', '214,112,218']
